...
const AppSidebar: React.FC = () => {
  ...
  const hasPermission = useCallback((item: NavItemProps) => {
    return hasAnyRole(item.restricted || []);
  }, [hasAnyRole]);
  ...
  const renderMenuItems = (
    ...
    <ul className="flex flex-col gap-4">
      {navItems.filter(hasPermission).map((nav, index) => (
        <li key={nav.name}>
          {nav.subItems ? (
            <button
              type="button"
  ...
  useEffect(() => {
    ...
    for (const menuType of ["main", "others"]) {
      for (const [index, nav] of menuItems.entries()) {
        if (nav.subItems) {
